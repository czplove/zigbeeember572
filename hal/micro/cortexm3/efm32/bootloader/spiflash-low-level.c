...
// default PA4 for power pin
  ...
  // Default to PA4 when board header doesn't fully define alternative
    ...
    #define CONFIGURE_EEPROM_POWER_PIN()  GPIO_PinModeSet(gpioPortA, 4, gpioModePushPull, 0)
    #define SET_EEPROM_POWER_PIN()        GPIO_PinOutSet(gpioPortA, 4)
    #define CLR_EEPROM_POWER_PIN()        GPIO_PinOutClear(gpioPortA, 4)
...
void halEepromConfigureFlashController(void){
  ...
  EXTERNAL_FLASH_SERIAL_CONTROLLER->ROUTELOC0 =   EXTERNAL_FLASH_USART0_TXLOC
                                                | EXTERNAL_FLASH_USART0_RXLOC
                                                | EXTERNAL_FLASH_USART0_CLKLOC ;
}
