...
static bool testReadWriteSanity(HalPydCfg_t *expectedCfg);
...
// Perform a read of the device and return true if all of the device reported
// configuration values match the given configuration structure
static bool testReadWriteSanity(HalPydCfg_t *expectedCfg)
{
  HalPydInMsg_t pydData;
  HalPydCfg_t observedPydCfg;

  pydData.config = &observedPydCfg;

  halOccupancyPyd1698Read(&pydData);

  if ((observedPydCfg.filterSource != expectedCfg->filterSource) ||
      (observedPydCfg.operationMode != expectedCfg->operationMode) ||
      (observedPydCfg.windowTime != expectedCfg->windowTime) ||
      (observedPydCfg.pulseCounter != expectedCfg->pulseCounter) ||
      (observedPydCfg.blindTime != expectedCfg->blindTime) ||
      (observedPydCfg.sensitivity != expectedCfg->sensitivity)) {
    return false;
  }
