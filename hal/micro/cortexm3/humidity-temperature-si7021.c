...
void emberAfPluginTemperatureSi7021ReadEventHandler(void)
{
  ...
  if (startReadBeforeInitialization) {
    ...
    if (errorCode != I2C_DRIVER_ERR_NONE) {
      ...
      //                     = (1000 * ( 5527 * ADC / 8192 - 11994 )) / 256
      temperatureCalculation = HIGH_LOW_TO_INT(buffer[0], buffer[1]);
      temperatureCalculation = 5527 * temperatureCalculation;
...
void emberAfPluginHumiditySi7021ReadEventHandler(void)
{
  ...
  if (startReadBeforeInitialization) {
    ...
    relativeHumidity = HIGH_LOW_TO_INT(buffer[0], buffer[1]);
...
//******************************************************************************
// Perform all I2C transactions necessary to read the humidity from the
// SI7021, convert it to milliDegrees Celsius, and return it.
//******************************************************************************
...
static uint8_t humidityAccuracySet(uint8_t measureAccuracy)
{
  ...
  writeBuffer[0] = SI_7021_USER_REG_READ;
  
  halI2cWriteBytes(SI_7021_ADDR, &writeBuffer[0], SEND_COMMAND_SIZE);
  errorCode = halI2cReadBytes(SI_7021_ADDR, &currentValue, SEND_COMMAND_SIZE);
  if (errorCode != I2C_DRIVER_ERR_NONE) {
    return errorCode;
  }
 
  writeBuffer[0] = SI_7021_USER_REG_WRITE;
  ...
  //                        |           |
  //                     ADCBit[1]   ADCBit[0]
  writeBuffer[1] = (currentValue & (0x7E)) |
              (((measureAccuracy & 0x2) << 6) | 
              ((measureAccuracy & 0x1)));
  errorCode = halI2cWriteBytes(SI_7021_ADDR, &writeBuffer[0], 2);
  return errorCode;
}
