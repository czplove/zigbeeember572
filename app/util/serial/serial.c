                                          ...
                                          uint8_t stopBits)
{
  ...
  if (port==COM_USART0)
  {
    initdata.uartdrvinit = (UARTDRV_Init_t) USART0_INIT;
  }
#endif
#ifdef COM_USART1_ENABLE
  if (port==COM_USART1)
  {
    initdata.uartdrvinit = (UARTDRV_Init_t) USART1_INIT;
  }
#endif
#ifdef COM_USART2_ENABLE
  if (port==COM_USART2)
  {
    initdata.uartdrvinit = (UARTDRV_Init_t) USART2_INIT;
  }
#endif
                                   ...
                                   uint8_t length)
{
  ...
  while (0 < length) {
    ...
    uint8_t bytes = (length < remainingInBuffer
                   ...
                   : remainingInBuffer);
    uint8_t i;
    emberSerialPrintf(2,"TX:");
    for (i = 0; i < bytes; i++)
    {
      emberSerialPrintf(2,"%2X ",*(emberMessageBufferContents(buffer)+start+i));
    }
    emberSerialPrintf(2,"\r\n");
