...
bool emberAfGreenPowerClientAppendEntryToResp(EmberGpProxyTableEntry * entry) {
  ...
  */
  ...
  send_notification: {
    ...
    if (commissioningState.unicastCommunication) {
      ...
      apsFrame->sourceEndpoint = GP_ENDPOINT;  //emberAfCurrentEndpoint();
      apsFrame->destinationEndpoint = GP_ENDPOINT; //emberAfCurrentEndpoint();
      EmberStatus retval = emberAfSendCommandUnicast(EMBER_OUTGOING_DIRECT,
                                                     commissioningState.commissioningSink);
      emSetAddDelay(0);
  ...
  for (i = 0; i < GP_SINK_LIST_ENTRIES; i++) {
    if (sinkList[i].type == EMBER_GP_SINK_TYPE_FULL_UNICAST
      ...
      if(rxAfterTx)
        ...
        emSetAddDelay(GP_DMIN_U);
      EmberStatus retval = emberAfSendCommandMulticastWithAlias(sinkList[i].groupcast.groupID,
                                                                emGpdAlias(addr),
                                                                sequenceNumber);
      emSetAddDelay(0);
