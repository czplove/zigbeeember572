...
// Results were recorded in stack/mac/802.15.4/Crypto-timing.xlsx
#if defined(EMBER_TEST)
  // Always turn this on in simulation so the code compiles. This helps insure it
  // won't break by another checkin.
  #define CRYPTO_TIMING_INFO
#endif
...
#define ENCRYPT_DECRYPT_OPERATION false
  ...
  static CryptoTimingInfo timingResults[NUMBER_OPERATIONS][NUMBER_RESULTS];
  static uint8_t  timingIndexes[NUMBER_OPERATIONS];

  static void startTiming(void);
  static void endTiming(CryptoPacketType type, 
                        bool micOperation, 
                        uint8_t packetSize,
                        uint8_t* auxFrame,
                        bool nwkOperation);
  #define RECORD_TIMING(packetType, micOperation, packetSize, auxFrame, nwkOperation, code) \
    do {                                                                         \
      ATOMIC(                                                                    \
        startTiming();                                                           \
        { code }                                                                 \
        endTiming(packetType, micOperation, packetSize, auxFrame, nwkOperation); \
      )                                                                          \
    } while(0)
  ...
  #if defined(EMBER_TEST)
    // We want to be able to compile and run this code for simulation even if the
    // timing is meaningless.  It is useful for debugging purposes of the crypto
    // timing code.  It also insures that it won't break accidentally.
    #define GET_INT32U_TIMER 0
  #else
    #define GET_INT32U_TIMER emGetMacTimer()
  #endif

#else  // !defined(CRYPTO_TIMING_INFO)

  #define RECORD_TIMING(packetType, micOperation, packetLength, auxFrame, nwkOperation, code)  { code }
  #define emCryptoTimingReset()
