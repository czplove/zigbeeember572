...
static uint16_t emAfPluginGbzMessageControllerAppendInPlace(EmberAfGbzMessageCreatorState * state,
                                                          EmberAfGbzZclCommand * zclCmd,
                                                          EmberAfGbzMessageData * data);
static uint16_t emAfPluginGbzMessageControllerGetZclLength(EmberAfGbzZclCommand * cmd);
                                                     ...
                                                     uint16_t gbzCommandLength)
{
  ...
  if (gbzCommand == NULL){ //inits for appends that requires mem allocation
    ...
    if (type == EMBER_AF_GBZ_MESSAGE_ALERT) {
	  ...
	  headerPayload[headerWriteIndex++] = (int8u) ((timestamp >> 24) & 0xFF);
	  headerPayload[headerWriteIndex++] = (int8u) ((timestamp >> 16) & 0xFF);
	  headerPayload[headerWriteIndex++] = (int8u) ((timestamp >> 8) & 0xFF);
	  headerPayload[headerWriteIndex++] = (int8u) (timestamp & 0xFF);
    }
    ...
    if (type == EMBER_AF_GBZ_MESSAGE_ALERT) {
	  ...
	  state->command[state->commandIndex++] = (int8u) ((timestamp >> 24) & 0xFF);
	  state->command[state->commandIndex++] = (int8u) ((timestamp >> 16) & 0xFF);
	  state->command[state->commandIndex++] = (int8u) ((timestamp >> 8) & 0xFF);
	  state->command[state->commandIndex++] = (int8u) (timestamp & 0xFF);
    }
...
static uint16_t emAfPluginGbzMessageControllerAppendInPlace(EmberAfGbzMessageCreatorState * state,
  ...
  uint16_t * extHeaderGbzCmdLen = NULL;
